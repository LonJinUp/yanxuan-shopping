// pages/SecondaryPage/Settlement/index.js
var app = getApp();
Page({
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          if (res.data.code == 200 && res.data.data.status == 'SUCCESS') {
            ...
            setTimeout(function () {
              wx.navigateTo({
                url: '/pages/LoadPay/index?order_id=' + data.result.orderId
              })
            }, 1200)
            ...
            wx.requestPayment({
              ...
              success: function (res) {
                ...
                setTimeout(function () {
                  wx.navigateTo({
                    url: '/LoadPay/index?order_id=' + data.result.orderId
                  })
                }, 1200)
              },
              fail: function (res) {
  ...
  getAddress: function () {
    ...
    wx.request({
      ...
      success: function (res) {
        if (res.data.code == 200) {
          wx.navigateTo({ //跳转至指定页面并关闭其他打开的所有页面（这个最好用在返回至首页的的时候）
            url: '/pages/Address/index?cartId=' + that.data.cartId + '&pinkId=' + that.data.pinkId + '&couponId=' + that.data.couponId
          })
        }
      }
    })
